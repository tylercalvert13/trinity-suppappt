...
const GHL_WEBHOOK_URL = Deno.env.get('GHL_WEBHOOK_URL_SUPPAPPT1')!;
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Received lead data (suppappt1):", JSON.stringify(data));
    ...
    const payload = {
      ...
      // Appointment details (new for suppappt1 - sent only on booking)
    ...
    console.log("Sending to GHL webhook (suppappt1):", GHL_WEBHOOK_URL);
