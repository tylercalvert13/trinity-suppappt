...
interface ExitIntentModalRefundProps {
  monthlySavings: number;
  onBookClick: () => void;
}

export function ExitIntentModalRefund({ monthlySavings, onBookClick }: ExitIntentModalRefundProps) {
  const [showModal, setShowModal] = useState(false);
  const [hasShown, setHasShown] = useState(false);

  useEffect(() => {
    // Check if already shown this session
    const alreadyShown = sessionStorage.getItem('exit_intent_shown_refund');
    if (alreadyShown) {
      setHasShown(true);
      return;
    }
    ...
    const triggerModal = () => {
      setShowModal(true);
      setHasShown(true);
      sessionStorage.setItem('exit_intent_shown_refund', 'true');
    };
