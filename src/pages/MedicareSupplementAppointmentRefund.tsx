...
import { ExitIntentModalRefund } from '@/components/ExitIntentModalRefund';
...
const trackBingSubmissionEvent = (formData: FormData) => {
  try {
    ...
    console.log('Bing UET submit_lead_form conversion tracked (suppappt-refund)');
  } catch (error) {
...
const trackGoogleAdsConversion = () => {
  try {
    ...
    console.log('Google Ads submit_lead_form conversion tracked (suppappt-refund)');
  } catch (error) {
...
const trackVibeCoLeadEvent = () => {
  try {
    ...
    console.log('Vibe.co lead event tracked (suppappt-refund)');
  } catch (error) {
    console.error('Error tracking Vibe.co lead event:', error);
  }
};

const MedicareSupplementAppointmentRefund = () => {
  ...
  const { visitorId, sessionId, trackStepChange, trackQualification, trackEvent } = useFunnelAnalytics('suppappt-refund');
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Send lead to GHL webhook (suppappt-refund specific)
      await supabase.functions.invoke('send-lead-webhook-suppappt-refund', {
        body: {
          ...formData,
          currentPayment: parseFloat(formData.currentPayment),
          age: parseInt(formData.age),
      ...
      <section className="bg-gradient-to-br from-blue-900 via-blue-800 to-blue-700 text-white py-8 md:py-12">
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="inline-flex items-center bg-red-600 text-white px-4 py-2 rounded-full text-sm font-bold mb-6 animate-pulse">
            ...
            EXPOSED: Medicare Supplement Overcharges
          </div>
          ...
          {(() => {
            ...
            if (isLoadingLocation && !formData.zipCode) {
              return (
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 leading-tight">
                  Seniors <span className="inline-block bg-white/20 rounded animate-pulse w-24 h-8 align-middle mx-1"></span> on Plan G, F, or N Qualify for $100-200/Month Back
                </h1>
              );
            }
            ...
            return (
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold mb-4 leading-tight">
                Seniors {stateText}on Plan G, F, or N Qualify for $100-200/Month Back
              </h1>
            );
          })()}
          
          <p className="text-lg md:text-xl text-blue-100 mb-4 max-w-2xl mx-auto">
            Your benefits are <span className="font-bold text-white">federally standardized</span> — if you're paying more than the lowest rate, you can get that money back.
          </p>
          
          <p className="text-base text-blue-200 mb-8 max-w-xl mx-auto">
            See how much you qualify to get back in under 2 minutes. No obligation.
          </p>
        ...
        <div ref={questionContainerRef} className="max-w-2xl mx-auto px-4 scroll-mt-4">
          ...
          {step === "treatment" && (
            <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border">
              ...
              <p className="text-lg md:text-xl text-muted-foreground mb-6">
                In the past 2 years, have you been diagnosed with or treated for any of these?
              </p>
              
              <ul className="text-foreground mb-8 space-y-3">
                <li className="flex items-start gap-3 text-lg md:text-xl">
                  <span className="text-blue-600 font-bold mt-0.5">•</span>
                  <span>Cancer (other than non-melanoma skin cancer)</span>
                </li>
                <li className="flex items-start gap-3 text-lg md:text-xl">
                  <span className="text-blue-600 font-bold mt-0.5">•</span>
                  <span>Heart attack or stroke</span>
                </li>
                <li className="flex items-start gap-3 text-lg md:text-xl">
                  <span className="text-blue-600 font-bold mt-0.5">•</span>
                ...
                <li className="flex items-start gap-3 text-lg md:text-xl">
                  <span className="text-blue-600 font-bold mt-0.5">•</span>
                  <span>Kidney failure or dialysis</span>
                </li>
                <li className="flex items-start gap-3 text-lg md:text-xl">
                  <span className="text-blue-600 font-bold mt-0.5">•</span>
          ...
          {step === "contact" && (
            <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border">
              ...
              <p className="text-xs text-muted-foreground text-center mt-4 leading-relaxed">
                By clicking "See What I Qualify For," I consent to receive calls, text messages, and emails 
...
export default MedicareSupplementAppointmentRefund;
